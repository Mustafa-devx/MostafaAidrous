...
function App() {
  ...
  return (
    <div className={`min-h-screen w-full overflow-hidden transition-all duration-500 ease-out bg-app-bg text-app-text`}>
      ...
      {/* Background Effects - Enhanced CSS Starfield with Nebula */}
      <div className="starfield-container">
        <div className="stars-nebula" />
        <div className="stars-layer" />
        <div className="stars-layer-mid" />
        <div className="stars-layer-slow" />

        {/* Subtle Theme-Specific Overlays - Optimized */}
        <motion.div
          style={{ transform: 'translateZ(0)' }}
          animate={{
            opacity: isDarkMode ? [0.04, 0.08, 0.04] : [0.01, 0.03, 0.01],
          }}
          transition={{ duration: 12, repeat: Infinity, ease: "linear" }}
          className={`absolute inset-0 blur-[150px] ${isDarkMode ? 'bg-violet-900/15' : 'bg-blue-200/5'}`}
        />
      </div>
    </div>
  );
